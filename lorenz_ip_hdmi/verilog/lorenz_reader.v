...
`timescale 1 ns / 1 ps
	...
	// ============================================================
	// >>> USER EDIT (start/stop + speed feature): PS-writable control
	// register (offset 0xC / slv_reg3).
	//
	// Bit 0 is a run/pause flag: the CPU writes 1 to start (or resume)
	// the solver and 0 to pause it, from software on the terminal.
	// Pausing holds every integrator's register in place (see
	// integrator.sv's "run" input) instead of clearing it, so resuming
	// continues from exactly those held values.
	//
	...
	// identical to the original always-on behavior.
	//
